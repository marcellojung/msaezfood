    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-actions>
            ...
            <v-btn
                    ...
                    color="deep-purple lighten-2"
                    text
                    @click="openAcceptoreject"
            >
                Acceptoreject
            </v-btn>
            <v-dialog v-model="acceptorejectDiagram" width="500">
                <AcceptorejectCommand
                        @closeDialog="closeAcceptoreject"
                        @acceptoreject="acceptoreject"
                ></AcceptorejectCommand>
            </v-dialog>
            <v-btn
                    v-if="!editMode"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async acceptoreject(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['acceptoreject'].href), params)
            ...
            openAcceptoreject() {
                this.acceptorejectDiagram = true;
            },
            closeAcceptoreject() {
                this.acceptorejectDiagram = false;
            },
            async start() {
                try {
                    if(!this.offline) {
